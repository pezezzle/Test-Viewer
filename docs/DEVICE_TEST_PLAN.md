...
3. **Persistence:** Fully close and reopen the app, then restart the Android device. Verify the stored grant and settings. Install a genuinely signed update over the existing version.
...
10. **Display:** Test phones and tablets, portrait/landscape, the on-screen keyboard, larger text, and TalkBack. The main page must scroll completely. Errors and long customer names must not be clipped.
...
12. **Release:** Test offline mode and verify the app name/icon, application ID, signature, and build number. Validate the Android AAB with the publisher's store tooling.
