...
void main() {
  for (final width in [320.0, 390.0, 1024.0]) {
    testWidgets('Dashboard and device page render at width $width', (tester) async {
      tester.view.physicalSize = Size(width, 1000);
      tester.view.devicePixelRatio = 1;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
      await tester.pumpWidget(TestMasterApp(store: DemoViewerStore(reference: const CalendarDay(2026, 9, 5))));
  ...
  testWidgets('Reference date can be changed and reset', (tester) async {
    ...
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const ValueKey('reference-date-input')), '2027-01-15');
    await tester.pumpAndSettle();
  ...
  for (final save in [false, true]) {
    testWidgets('Source settings safely ${save ? "save" : "close"} with a focused input', (tester) async {
      await tester.pumpWidget(TestMasterApp(store: DemoViewerStore()));
      await tester.pumpAndSettle();
      await tester.tap(find.widgetWithText(OutlinedButton, 'Datenbank'));
      await tester.pumpAndSettle();
      await tester.enterText(find.byKey(const ValueKey('database-path')), 'example.sqlite3');
      final action = save
          ? find.widgetWithText(FilledButton, 'Pfad speichern und laden')
  ...
  testWidgets('Location search allows multiple checked results', (tester) async {
    await tester.pumpWidget(TestMasterApp(store: DemoViewerStore()));
    await tester.pumpAndSettle();
    await tester.tap(find.widgetWithText(OutlinedButton, 'Alle Standorte'));
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const ValueKey('location-search')), 'Raum');
    await tester.pumpAndSettle();
